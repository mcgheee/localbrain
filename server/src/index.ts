...
function createMcpServer(): McpServer {
  ...
  server.registerTool(
    ...
    async ({ query, limit, threshold }) => {
      try {
        ...
        const result = await pool.query<ThoughtRow & { similarity: number }>(
          `
            SELECT
              content,
              metadata,
  ...
  server.registerTool(
    ...
    async ({ limit, type, topic, person, days }) => {
      try {
        ...
        const result = await pool.query<ThoughtRow>(query, values);
        if (result.rows.length === 0) {
  ...
  server.registerTool(
    ...
    async () => {
      try {
        const countResult = await pool.query<{ count: string }>("SELECT COUNT(*)::text AS count FROM thoughts");
        const rowsResult = await pool.query<Pick<ThoughtRow, "created_at" | "metadata">>(
          "SELECT created_at, metadata FROM thoughts ORDER BY created_at DESC",
        );
...
async function extractMetadata(text: string): Promise<ThoughtMetadata> {
  ...
  const parsed = safeParseJson<ThoughtMetadata>(rawContent);
  if (!parsed) {
    return { topics: ["uncategorized"], type: "observation" };
  }
